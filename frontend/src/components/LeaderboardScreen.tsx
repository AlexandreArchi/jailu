...
export default function LeaderboardScreen({ myProfile, friends, onClose }: Props) {
  ...
  }, [allBooks, selectedYear, metric, myProfile, friends])
                    ...
                    <div className="relative flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className={`truncate text-sm font-semibold ${player.isMe ? 'text-indigo-200' : 'text-white'}`}>
                          @{player.username}{player.isMe && <span className="ml-1 text-[10px] font-medium text-indigo-400">toi</span>}
                        </p>
                      </div>
                      {/* Value */}
